...
/**
 * Bashir&Co — a piece's own page (RU).
 *
...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  const piece = await getPublicPieceBySlug(slug, "ru");
  ...
  const description =
    piece.description ??
    `${piece.brand} — ${piece.title}. ${category} под заказ с доставкой в Алматы. Проверяем подлинность, состояние и комплект.`;
...
export default async function PiecePage({ params }: Params) {
  const { slug } = await params;
  const all = await getPublicPieces("ru");
  ...
  return (
    <PieceView locale="ru" dictionary={d} piece={piece} related={related} />
  );
}
